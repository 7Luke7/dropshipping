...
const Addresses = () => {
    ...
    const delete_address = async (id) => {
        try {
            const request = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/address/500`, {
                method: "DELETE",
                credentials: "include"
            })
    ...
    <Helmet>
      <title>Slashy - ჩემი მისამართები</title>
    </Helmet>
    ...
    {addresses && addresses.length > 0 ?
      ...
      </div> : addresses && addresses.length === 0 && <div className="flex w-full justify-center items-center flex-col">
