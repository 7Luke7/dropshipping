...
export const SingleCarouselItem = memo(({image, i, setCurr, curr}) => {
    return <div onClick={() => {
        ...
        {image && image.includes('.mp4') && <button className="w-7 h-7 outline-none rounded-full bg-[rgb(251,77,1)] z-50 opacity-50 flex justify-center items-center absolute top-1/2 bottom-1/2 left-1/2 right-1/2 -translate-y-1/2 -translate-x-1/2">
            <img loading="lazy" src={smallVideoPlayIcon} width={5} height={5} className="h-[24px] w-[24px]" alt="ჩართვა"></img>
        </button>}
        <img
            loading="lazy"
            className={`${i === curr.index && "outline outline-1 outline-[rgb(251,77,1)]"} xxs:border rounded object-contain h-full w-full`}
